...
##                RGB colour image.  Returns a 24-bit image with the selected
##                channel included and 0's everywhere else.  Useful for colour
##                display of a single image channel, but if you just want the
##                channel as an 8-bit image, don't use this, just use im[...,i]
##                where i is the channel number you want to select.
##
##
...
def imshow(im, autoscale=False,colourmap='gray', newfig=True, title=None,
    ...
    """
    if newfig: 
        if title != None: fig = pyp.figure(title, figsize=figsize)
        else: fig = pyp.figure(figsize=figsize)
    if autoscale:
...
def getChannel(im, channel='R'):
    """Pull the red (R), green (G) or blue (B) channel from an RGB image
       *im*.  Returns an RGB image with the other two channels zeroed.  Useful
       for display of a single channel in colour, but if you just want the
       byte values of that channel, don't use this, instead just use 
       im[...,i], where i is the number of the channel you want to extract
    ...
    R,C = im.shape[:2]
    chImage = np.zeros((R,C,3), dtype = im.dtype)
...
##
##  Just CLAHE, imread and imshow for the moment.
##
